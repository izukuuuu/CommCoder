...
def main() -> None:
    parser = argparse.ArgumentParser(description="生成词云图片，基于后端已有数据")
    parser.add_argument("--session", help="指定会话 ID，默认选择最近使用的会话")
    parser.add_argument("--data-path", help="直接从文件加载数据 (.pkl/.xlsx/.csv)")
    parser.add_argument("--dimension", choices=["topic", "field"], default="topic", help="词云维度：topic 或 field")
    ...
    if not categories:
        raise RuntimeError("当前数据集中暂无可用于词云的分类信息")
    ...
    if not isinstance(words, list) or not words:
        raise RuntimeError("目标分类缺少词频数据，无法生成词云")

    assets = backend._generate_word_cloud_assets(words)  # type: ignore[attr-defined]
